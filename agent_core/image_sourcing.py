...
from branded_card import generate_branded_card
...
def _download_image(url: str, output_dir: str) -> str:
    ...
    filename = f"real_image_{int(time.time() * 1000)}{ext}"
...
def _try_reference_images(reference_images: list, output_dir: str):
...
def get_image_for_post(search_query: str, optimized_text: str, spec: PlatformSpec,
                        ...
                        reference_images: list = None, visual_brief: VisualBrief = None) -> dict:
    """
    ...
    persistent file hosting (e.g. a serverless deployment).
    ...
    path, source_url = _try_reference_images(reference_images, output_dir)
    if path:
        return {"image_path": path, "source": "reference_link", "source_url": source_url,
                "illustration_prompt": None, "visual_style": style_label, "visual_category": category}

    path, source_url = _web_search_image(effective_query, output_dir)
    if path:
        return {"image_path": path, "source": "web_search", "source_url": source_url,
                "illustration_prompt": None, "visual_style": style_label, "visual_category": category}
